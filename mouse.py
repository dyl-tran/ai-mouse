...
while cap.isOpened():
    ...
    if res.multi_hand_landmarks:
        ...
        if not scrolling_down:  # Only smooth if not scrolling down
            x_smooth = alpha * x_ix + (1 - alpha) * \
                prev_x if prev_x is not None else x_ix
            y_smooth = alpha * y_ix + (1 - alpha) * \
                prev_y if prev_y is not None else y_ix
        else:
            # Keep previous coordinates if scrolling down
            x_smooth, y_smooth = prev_x, prev_y
        ...
        # Display box range
        cv2.rectangle(image, (150, 150), (camW - 150,
                      camH - 150), (255, 0, 100), 2)
